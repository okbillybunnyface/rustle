...
impl Word {
    ...
    fn rand() -> Word {
        ...
        let word: String = words[rng.gen_range(0..words.len())].to_lowercase();
        //println!("{}",word);
        Word::new(word)
    }
    fn score_string(&self) -> String {
        ...
        for s in &self.score {
            match s {
                Score::Wrong => output.push_str("⬜"),
                Score::Miss => output.push_str("🟨"),
                Score::Hit => output.push_str("🟩"),
...
impl Turn {
    fn new(target: &Word) -> Turn {
        ...
        let mut word: Word = Word::new(guess);
        word.score(target);
        Turn::Guess(word)
    }
    fn is_win(&self) -> bool {
    ...
    fn word_string(&self) -> String {
        match self {
            Turn::Guess(word) => return word.word_string(),
            Turn::Nil => return String::new(),
        }
    }
}
...
fn main() {
    ...
    while game.state == State::Playing {
        ...
        for t in &game.turns {
            match &t {
                &Turn::Guess(_word) => println!("{} {}", t.score_string(), t.word_string()),
                &Turn::Nil => print!(""),
            }
        }
        println!("");
